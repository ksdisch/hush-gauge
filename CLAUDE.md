...
- **Frozen artifacts are hash-checked by the gate code**, not trusted from the caller;
  gates enforce the held-out split themselves (D7, D8), and **recompute every reported rate
  from the trials** rather than deciding on caller-supplied aggregates (D14). The result JSON
