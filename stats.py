"""stats.py — proportion confidence intervals.

**Ported verbatim from `~/Projects/mute-map/stats.py`** (itself from lossy-wall, from
decay-pin) per the house rule that runners are cut from their predecessor. The three
functions are unchanged; only the call-site examples below name hush-gauge's arms.

Every headline number in this project is a **proportion**: k hits out of n trials — an
emission rate per (tier × scale) cell, a workspace-entry rate on non-emitting trials, a
post-ablation emission rate. A proportion is NOT a normal "average," so the familiar mean ± standard-
...
methodology guardrail in code form:
  ...
  - `wilson(k, n)` — the **Wilson score interval**: the honest range a single cell's *true*
    ...
    edges too: a T0 emission rate may sit at ≈0% while a 0.5B T3 rate sits near ceiling
    (R4). k=0 at n=25 reads [0%, ~13%] — "consistent with ~0%", never "proved 0%".
    **The anchors this project compares against are our own recorded numbers** from
    dim-stage and mute-map, never a paper claim.

  - `newcombe_diff(...)` — the **Newcombe interval** for the *difference* between two
    cells' rates. This is the number the gates turn on: G0 is (T4 − T0) excluding zero on
    the **secret-level** rate (k of 25), G2 is (silent-leak rate − yardstick rate), G3 is
    (pre-ablation − post-ablation emission rate). If the interval straddles 0, we cannot
    claim a gap — `excludes_zero(...)` is that test.

    **The hush-gauge caveat, owned in `D1`/`D8`, not papered over:** `newcombe_diff` is
    documented for two **independent** proportions, and G0's two arms are **paired** —
    both cells are computed over the same 25 held-out secrets. No level of aggregation
    makes the independence assumption true here. The pairing makes the
    independent-samples interval **conservative** under positive correlation, which is
    the acceptable direction for a gate; `GATE_WORDING` states this as a limitation
    rather than claiming it as a property.
...
def newcombe_diff(
    ...
    for arm 2. Call sites name the arms: G0 passes base=T0, mech=T4 (and base=T0,
    mech=T1/T2/T3/T4_turn1 for the exposure-matched companions `D3` makes mandatory).
    """
